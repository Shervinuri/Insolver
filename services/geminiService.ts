...
const formatHistoryForGemini = (history: Message[]): Content[] => {
    // Filter out the initial HTML report and format for Gemini
    return history
        .filter(msg => !msg.content.startsWith('<!DOCTYPE html'))
        .map(msg => ({
            role: msg.role,
...
export const generateWithGemini = async (prompt: string, history: Message[]): Promise<string> => {
    console.log("Using Google Gemini API for iNFORMA SHΞN™...");
    
    // Exclude the current user prompt from the history passed to the chat instance,
    // as it will be sent via sendMessage.
    ...
    try {
        ...
        let errorMessage = "Sorry, I encountered an error while connecting to the AI service. Please check the console for more details.";
        if (error instanceof Error && error.message.includes('API key not valid')) {
            errorMessage = "There seems to be an issue with the API key configuration. Please contact the administrator.";
        } else if (error instanceof Error) {
            errorMessage = `An error occurred: ${error.message}. Please check the console.`;
        }
